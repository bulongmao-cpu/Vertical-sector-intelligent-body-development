...
def save_markdown_to_docx(md_text, path, image_map=None, summary=None):
    ...
    style.font.name = 'Time New Romen'
...
def segment_by_area(df, split_col, target_cols, method="tree", max_segments=None, min_ratio=0.3):
    ...
    best_labels = None
        ...
        tree.fit(X, y)
        labels = tree.apply(X)
        df["原始标签"] = labels  # 保留原始的叶节点标签

        # 计算每个标签对应的 split_col 均值（即面积均值）
        label_means = df.groupby("原始标签")[split_col].mean()

        # 按面积均值排序标签，构建映射：最小面积标签 → 0，依此类推
        sorted_labels = label_means.sort_values().index.tolist()
        label_mapping = {old_label: new_label for new_label, old_label in enumerate(sorted_labels)}

        # 应用映射
        df["分段标签"] = df["原始标签"].map(label_mapping)
        ...
        target_bounds = [50000, 150000]  # 预期边界（平方米）
        search_ranges = [
            np.linspace(target_bounds[0] - 5000, target_bounds[0] + 5000, 11),  # [4万, 6万]
            np.linspace(target_bounds[1] - 15000, target_bounds[1] + 15000, 31)   # [14万, 16万]
        ]
        ...
        for b1 in search_ranges[0]:
            for b2 in search_ranges[1]:
                if b1 >= b2:
                    continue
                groups = [
                    df[df[split_col] <= b1],
                    df[(df[split_col] > b1) & (df[split_col] <= b2)],
                    df[df[split_col] > b2]
                ]
                score = calc_score(groups)
                if score > best_score:
                    best_score = score
                    best_split = [round(b1, 4), round(b2, 4)]
                    labels = []
                    for val in df[split_col]:
                        if val <= b1:
                            labels.append(0)
                        elif val <= b2:
                            labels.append(1)
    ...
    for i in range(len(bins) - 1):
        ...
        interval_labels.append(f"{left}-{right}平方米")
    ...
    # 重置列顺序（防止乱序）
    summary_df = summary_df[[split_col+"划分区间", "目标列", "均值"]]
...
def plot_scatter(df, split_col, target_col, thresholds, img_path):
    ...
    colors = plt.cm.get_cmap("tab10", len(unique_segments))
...
def 区间划分(main_input, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request):
    ...
    # 从 plugin_kwargs 获取最大分段数
    raw_input = plugin_kwargs.get("sub_input")
    print("[DEBUG] 用户输入的最大分段数为：", raw_input)  # ✅ 打印到后台
    # 从 plugin_kwargs 中提取最大分段数（带默认值）
    max_segments = int(plugin_kwargs.get("sub_input", 2))
    # max_segments = 2  # 最大分段数

    min_ratio = 0.5 / max_segments  # 最小分段比例

    df = pd.read_excel(filepath)
    chatbot.append(["读取文件", filepath])
    target_cols = [col for col in df.columns if str(col).startswith("Y")]
    # target_cols = ["单位面积成本", "人均单位成本", "平均服务面积", "保洁成本（2021）"]
    matched_split_cols = [col for col in df.columns if str(col).startswith("X")]
    if len(matched_split_cols) == 1:
        split_col = matched_split_cols[0]
        chatbot.append(["核心动因字段：",split_col])
    else:
        raise ValueError(f"匹配到多个划分字段：{matched_split_cols}")
    chatbot.append([f"找到划分依据字段：", str(target_cols)])
    ...
    segmented_df, breakpoints, summary ,summary_ord= segment_by_area(
        ...
        method="tree",  # 可换成"maxdiff", "tree", "jenks",fixed
        max_segments=max_segments,
        min_ratio=min_ratio
    )
    # chatbot.append(["分段结果：", segmented_df["分段标签"].value_counts()])
    ...
    summary_prompt = f"""
你是一位擅长数据分析的财务专家，请基于以下输入数据，总结**在核心动因上进行区间划分下的成本趋势**，并用专业、精准的语言撰写一段分析性结论。

请注意以下撰写要求：
1. **不要重复相同的句子**；
2. 使用合适的过渡词，增强逻辑连贯性；
3. 不使用口语化或模板式表达，如“如下所示”“见上表”“明显可以看出”等；
4. 输出为文字分析（不使用列表或分点形式），适合写入正式分析报告。

你应描述以下内容：
- 明确指出不同区间的显著差异或趋势变化。
- 根据输入数据在【{split_col}】字段上的划分范围，说明各区间的划分范围并在【{target_cols}】指标上的平均值。
- 指出各区间在划分依据字段上的异同；并识别可能的规模效益（如面积大 → 单位成本低等）。
- 输出应为一段文字分析，适合用于分析报告。
...
"""
    ...
    res = write_history_to_file(history, file_basename="区间划分报告.md")
